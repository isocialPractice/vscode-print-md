...
const cleanTemp_1 = require("./utils/cleanTemp");
// Global variables.
var extContext; // defined at activate
// ************************************* SUPPORT FUNCTIONS *************************************
...
function activate(context) {
    ...
    try {
        (0, child_process_1.execSync)(`${pythonCmd} --version`, { stdio: 'ignore' });
    }
    catch (error) {
        vscode.window.showWarningMessage(`Python not found. The Print Markdown extension requires Python 3.x to be installed. ` +
    ...
    let lastUsedPrinter = context.globalState.get('lastUsedPrinter');
    ...
    let disposable = vscode.commands.registerCommand('printMD.print', async () => {
        ...
        if (editor && editor.document.languageId === 'markdown') {
            ...
            const mdEditors = vscode.window.visibleTextEditors.filter(e => e.document.languageId === 'markdown');
            if (mdEditors.length > 0) {
                mdDocument = mdEditors[0].document;
            }
            else {
                // Check all open documents (including those not currently visible)
                const mdDocs = vscode.workspace.textDocuments.filter(doc => doc.languageId === 'markdown' && !doc.isUntitled);
                if (mdDocs.length > 0) {
                    mdDocument = mdDocs[0];
                }
            }
        ...
        try {
            ...
            const html = await vscode.commands.executeCommand('markdown.api.render', mdDocument.getText());
            if (!html) {
            ...
            (0, cleanTemp_1.cleanTempPrintFolder)(tempDir);
            ...
            const panel = vscode.window.createWebviewPanel('markdownPrintPreview', `Print Preview: ${path.basename(mdDocument.fileName)}`, vscode.ViewColumn.Beside, {
                enableScripts: true
            });
            ...
            panel.webview.onDidReceiveMessage(async (message) => {
                if (message.command === 'debug') {
                    ...
                    const pyProc = (0, child_process_1.spawn)(command, [], { shell: true });
                    let printerOutput = '';
                    let errorOutput = '';
                    ...
                    const printerSelectMode = config.get('printer.select', 'default');
                    ...
                    if (isPdfMode) {
                        const defaultPath = path.join(path.dirname(mdDocument.fileName), path.basename(mdDocument.fileName, '.md') + '.pdf');
...
function createPrintableHtml(bodyHtml, title) {
